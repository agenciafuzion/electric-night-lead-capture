...
const JobCard = ({ job, index }: { job: Job; index: number }) => {
  ...
  const postedLabel = `Publicada ${formatDistanceToNow(new Date(job.created_at), { addSuffix: true, locale: ptBR })}`;
  const companyLabel = job.is_confidential ? "Confidencial" : "Loop Consultoria";
      ...
      >
        ...
        <div className="relative p-6 md:p-8 flex flex-col flex-1">
          <div className="flex items-start justify-between gap-4 mb-5">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="font-body text-sm text-neutral-mid">{companyLabel}</span>
                {job.is_urgent && (
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            ...
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-depth text-neutral-mid font-body text-xs">
              <Clock className="w-3.5 h-3.5" /> {job.contract_type}
            </span>
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-depth text-neutral-mid font-body text-xs">
              <Briefcase className="w-3.5 h-3.5" /> {job.seniority}
            </span>
            {job.salary_range && (
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {job.tags.map((tag) => (
              <span key={tag} className="px-3 py-1 rounded-full border border-electric/20 text-electric font-body text-xs">
                {tag}
              </span>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between pt-5 border-t border-depth">
            <span className="font-body text-xs text-neutral-mid">{postedLabel}</span>
        ...
        <DialogContent className="max-w-2xl bg-deep border-depth text-primary-foreground p-0 rounded-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
          <div className="p-6 md:p-8">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              ...
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-depth text-neutral-mid font-body text-sm">
                <Clock className="w-4 h-4" /> {job.contract_type}
              </span>
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-depth text-neutral-mid font-body text-sm">
                <Briefcase className="w-4 h-4" /> {job.seniority}
              </span>
              {job.salary_range && (
            ...
            <div className="mb-8">
              ...
              <div className="flex flex-wrap gap-2">
                {job.tags.map((tag) => (
                  <span key={tag} className="px-4 py-1.5 rounded-full border border-electric/20 text-electric font-body text-sm">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            <div className="flex items-center justify-between pt-6 border-t border-depth">
              <span className="font-body text-sm text-neutral-mid">{postedLabel}</span>
              {job.application_url ? (
                <a
                  href={job.application_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-lg bg-electric text-primary-foreground font-body text-sm font-semibold hover:opacity-90 transition-opacity"
                >
